...
def getTermWeights(pixels, mask, certainBgMask):
	...
	invCovs = np.linalg.inv(GMM.covariances_)
	sqrtDetCovs = np.sqrt(np.linalg.det(GMM.covariances_))
...
def getNewFgBg(height, width, neighborWeightsY, neighborWeightsX, neighborWeightsDiag1, neighborWeightsDiag2, fgWeights, bgWeights):
	numPixels = height * width
	g = maxflow.Graph[float](numPixels, 2 * numPixels * NUM_NEIGHBORS)
